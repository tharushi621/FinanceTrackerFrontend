...
export default function Header({ theme }) {
  ...
  return (
    <header className="bg-white border-b px-6 py-3 flex items-center justify-between shadow-sm"
      style={{ borderColor: theme?.cardBg || '#e2e8f0' }}>
      <div className="flex items-center gap-6">
        ...
        <div className="hidden md:flex items-center gap-2 px-4 py-2 rounded-xl"
          style={{ background: theme?.cardBg || '#dcfce7', border: `1px solid ${theme?.accent}30` }}>
          <Clock size={14} style={{ color: theme?.accent || '#16a34a' }} />
          <div>
            <p className="text-sm font-bold" style={{ color: theme?.accent || '#15803d' }}>{formatTime(time)}</p>
            <p className="text-xs opacity-70" style={{ color: theme?.text || '#16a34a' }}>{formatDate(time)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={logout}
          ...
          style={{ background: `linear-gradient(135deg, ${theme?.text || '#064e3b'}, ${theme?.accent || '#059669'})` }}>
          <LogOut size={14} />
          Logout
        </button>
      </div>
